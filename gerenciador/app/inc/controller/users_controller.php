...
class users_controller
{
	...
	{
		$done = array();
		$filter = array(" active = 'yes' ", " idx in ( select users_profiles.users_id from users_profiles where users_profiles.active = 'yes' and users_profiles.profiles_id in (1, 2, 3, 4, 5, 6, 7) ) ", "idx in ( select users_companies.users_id from users_companies where users_companies.active = 'yes' and users_companies.companies_id = '" . $_SESSION[constant("cAppKey")]["companie_id"] ."' ) ");

		if (!in_array($_SESSION[constant("cAppKey")]["credential"]["profiles_attach"][0]["idx"], array(1, 2)) && !in_array($_SESSION[constant("cAppKey")]["credential"]["profiles_attach"][0]["slug"], array('master', 'gestor-hsol'))) {
	...
	public function form($info)
	{
		...
		$companies->set_filter(array(" active = 'yes' "));
		$companies->load_data();
		$data2 = $companies->data;
		...
		if (isset($info["idx"])) {
			...
			$users->attach(array("companies"), null, null, array("idx"));
			$data = current($users->data);
			$companies_id = array_column($data["companies_attach"],'idx');
