...
class partners_controller
{
	...
	public function display($info)
	{
		...
		$partners->set_filter($filter);
		$partners->set_order(array($ordenation));
		list($total, $data) = $partners->return_data();
		$data = $partners->data;
		...
		switch ($info["format"]) {
			...
			default:
				...
				$form = array(
					"done" => rawurlencode(!empty($done) ? set_url($GLOBALS["partners_url"], $done) : $GLOBALS["partners_url"]),
					"pattern" => array(
						"new" => $GLOBALS["newpartner_url"],
						"action" => $GLOBALS["partner_url"],
						"search" => !empty($info["get"]) ? set_url($GLOBALS["partners_url"], $info["get"]) : $GLOBALS["partners_url"]
					)
				);
				...
				include(constant("cRootServer") . "ui/page/partners/partners.php");
				include(constant("cRootServer") . "ui/common/footer.inc.php");
				include(constant("cRootServer") . "ui/common/list_actions.php");
				print('<script>' . "\n");
				include(constant("cRootServer") . "furniture/js/partners/partners.js");
				print('</script>' . "\n");
	...
	public function form($info)
	{
		...
		if (isset($info["idx"])) {
			...
			$partner->set_filter(array(" idx = '" . $info["idx"] . "' "));
			$partner->load_data();
			$data = current($partner->data);
			$form = array(
				"title" => "Editar Sócio",
				"url" => sprintf($GLOBALS["partner_url"], $info["idx"])
			);
		} else {
			$data = array();
			$form = array(
				"title" => "Cadastrar Sócio",
				"url" => $GLOBALS["newpartner_url"]
			);
		}
		...
		include(constant("cRootServer") . "ui/common/header.inc.php");
		include(constant("cRootServer") . "ui/common/head.inc.php");
		include(constant("cRootServer") . "ui/page/partners/partner.php");
		include(constant("cRootServer") . "ui/common/footer.inc.php");
		print("<script>");
		include(constant("cRootServer") . "furniture/js/partners/partner.js");
		print('</script>' . "\n");
		include(constant("cRootServer") . "ui/common/foot.inc.php");
	}
	...
	public function save($info)
	{
		...
		if (!isset($info["idx"]) || (int)$info["idx"] == 0) {
			$info["idx"] = $partner->con->insert_id;
		}

		if (isset($info["post"]["done"]) && !empty($info["post"]["done"])) {
			basic_redir($info["post"]["done"]);
		} else {
			basic_redir($GLOBALS["partners_url"]);
		}
	}
	...
	public function select_partners($info)
	{
		if (!site_controller::check_login()) {
			basic_redir($GLOBALS["home_url"]);
		}
		...
		include(constant("cRootServer") . "ui/common/header.inc.php");
		include(constant("cRootServer") . "ui/common/head.inc.php");
		include(constant("cRootServer") . "ui/page/partners/includes/select_partners.php");
		include(constant("cRootServer") . "ui/common/footer.inc.php");
		include(constant("cRootServer") . "ui/common/list_actions.php");
		print('<script>' . "\n");
		include(constant("cRootServer") . "furniture/js/partners/select_partners.js");
		print('</script>' . "\n");
		include(constant("cRootServer") . "ui/common/foot.inc.php");
	}
}
